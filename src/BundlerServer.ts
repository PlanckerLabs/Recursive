...
import { erc4337RuntimeVersion } from './Version'
import { deepHexlify } from './ERC4337Utils'

import { BundlerConfig } from './BundlerConfig'
import { UserOpMethodHandler } from './UserOpMethodHandler'
import { Server } from 'http'
import { RpcError } from './utils'
import { UserOperationStruct } from 'soulwallet-contract/src/types/contracts/interfaces/IEntryPoint'
import { EntryPoint__factory } from 'soulwallet-contract/src/types'
...
export class BundlerServer {
  ...
  async _preflightCheck (): Promise<void> {
    ...
    const emptyUserOp: UserOperationStruct = {
      ...
      callData: '0x',
      initCode: ethers.constants.AddressZero,
      paymasterAndData: '0x',
      nonce: 0,
    ...
    const err = await EntryPoint__factory.connect(this.config.entryPoint, this.provider).callStatic.simulateValidation(emptyUserOp)
      .catch(e => e)
    if (err?.errorName && err?.errorName !== 'FailedOp') {
  ...
  intro (req: Request, res: Response): void {
    res.send(`Recursive Bundler v.${erc4337RuntimeVersion}. please use "/rpc"`)
  }
